...
public class TimeUtil {
  ...
  private static final DateTimeFormatter DEFAULT_DATE_TIME_FORMATTER =
      DateTimeFormatter.ofPattern(DEFAULT_PATTERN).withLocale(java.util.Locale.CHINA);
  ...
  public static String format(long timestamp) {
    return DEFAULT_DATE_TIME_FORMATTER.format(
        new Date(timestamp).toInstant().atZone(DEFAULT_ZONE_ID));
  }
  ...
  public static Date parse(String time) {
    return Date.from(
        LocalDateTime.parse(time, DEFAULT_DATE_TIME_FORMATTER).atZone(DEFAULT_ZONE_ID).toInstant());
  }
